...
class Calculator(QMainWindow):
    ...
    def initUI(self):
        ...
        buttons = [
            ...
            ("0", 5, 0, "darkgray", 1, 2), (".", 5, 2, "darkgray"), ("=", 5, 3, "orange")
        ]
        ...
        for btn in buttons:
            ...
            button.setStyleSheet(f"""
                QPushButton {{ 
                    ...
                    border-radius: {'50%' if text != '0' else '25%'}; 
                    border: none; 
                    padding: 20px;
                }}
                QPushButton:pressed {{ 
    ...
    def calculate(self, button_text):
        if button_text == "AC":
            ...
            self._perform_convert()
        elif button_text in ["+", "−", "×", "÷", "%"]:
            self._perform_operator(button_text)
        elif button_text == "=":
            self._perform_calculator()
        elif button_text == ".":
            self._perform_decimal_point()
        else:
            self._perform_input_number(button_text)
        self.display.setText(self._display_text if self._display_text else self._result)
